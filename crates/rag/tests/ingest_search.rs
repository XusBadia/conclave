...
async fn ingest_three_fixtures_then_search_each() {
    ...
    let report = pipeline.ingest_path(&fixtures_dir(), |_| {}).await.unwrap();
    assert!(
    ...
    for query in ["furosemida", "metformina", "tiazidicos"] {
        ...
        let hits = pipeline.repository().search(&vec[0], 5).await.unwrap();
        assert!(
...
async fn round_trip_ingest_then_remove_each_fixture() {
    ...
    assert!(
        listed.is_empty(),
        "every document should be gone, got {listed:?}"
    );
}
